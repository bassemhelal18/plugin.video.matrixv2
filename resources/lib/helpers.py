...
def sort_sources_list(sources):
    if len(sources) > 1:
        try:
            ...
            log(r'Scrape sources sort failed |int(re.sub("\D", "", x[0])|')
            try:
                sources.sort(key=lambda x: re.sub("[^a-zA-Z]", "", x[0].lower()))
            except:
                log('Scrape sources sort failed |re.sub("[^a-zA-Z]", "", x[0].lower())|')
    return sources
...
def scrape_sources(html, result_blacklist=None, scheme='http', patterns=None, generic_patterns=True):
    ...
    def __parse_to_list(_html, regex):
        ...
        if matches:
            log('Scrape sources |%s| found |%s|' % (regex, matches))
        return matches
...
def pick_source(sources, auto_pick=None):
    ...
    if len(sources) == 1:
        ...
        if auto_pick:
            ...
            if result == -1:
                raise log(('no_link_selected'))
            else:
                return sources[result][1]
    else:
        raise log(('no_video_link'))
