...
# Domain Abfrage
DOMAIN = cConfig().getSetting('plugin_'+ SITE_IDENTIFIER +'.domain', 'ag31.arlionztv.art')
...
def showEntries(entryUrl=False, sGui=False, sSearchText=False):
    ...
    for sUrl, sName,sThumbnail  in aResult:
        ...
        if "سيرفر"  in sName:
                continue
        sName = sName.replace('مترجمة','').replace('مترجم','').replace('فيلم','').replace('مشاهدة','').replace('مسلسل','').replace('اون','').replace('أون','').replace('لاين','').split('الموسم')[0].split('الحلقة')[0].replace('سلسل','').strip()
        sYear=''
