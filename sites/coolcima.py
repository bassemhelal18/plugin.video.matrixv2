...
# Domain Abfrage
DOMAIN = cConfig().getSetting('plugin_'+ SITE_IDENTIFIER +'.domain', 'cooolman.store')
...
def showEpisodes():
    ...
    sPattern = 'href="([^"]+)".+?<span>الحلقة</span>(.+?)</div>'  # start element
                ...
                for sHosterID ,serverId  in aResult:
                 ...
                 if isMatch:
                  for sUrl4  in aResult:
                    if 'megamax' in sUrl4 or 'lodcima.shop' in sUrl4 :
