...
# Domain Abfrage
DOMAIN = cConfig().getSetting('plugin_'+ SITE_IDENTIFIER +'.domain', 'w.cimalight.co')
URL_MAIN = 'https://' + DOMAIN + '/'


URL_MOVIES_English = URL_MAIN + 'category.php?cat=english-movies'
URL_MOVIES_Arabic = URL_MAIN + 'category.php?cat=arabic-movies2'
URL_SERIES_English = URL_MAIN + 'category.php?cat=english-series1'
URL_SERIES_Arabic = URL_MAIN + 'category.php?cat=arabic-series7'
URL_MOVIES_Kids = URL_MAIN + 'category.php?cat=anime-movies'
Ramadan = URL_MAIN + 'category.php?cat=ramadan-series'
...
def showHosters():
    ...
    if isMatch:
       for shost in aResult :
        if 'larhu.com' in shost:
           sRefer = '|user-agent='+common.RAND_UA +'&referer=' +shost.split('play')[0]+'&verifypeer=false'
           oRequest = cRequestHandler(shost)
           sHtmlContent3 = oRequest.request()
           sStart = '<div class="download-box">'
           sEnd = '</div>'
           sHtmlContent4 = cParser.abParse(sHtmlContent3, sStart, sEnd)
           sPattern = 'href="(.*?)".*?class="btn btn-quality">.*?Download Quality.*?(.*?p)'
           isMatch,aResult = cParser.parse(sHtmlContent4, sPattern)
           if isMatch:
            for shost,sQuality in aResult :
             sName = cParser.urlparse(shost)
             sName =  sName.split('.')[-2]
             hoster = {'link': shost + sRefer, 'name': sName, 'displayedName':sName+' '+sQuality, 'quality': sQuality, 'resolveable': True, 'resolved': True}
